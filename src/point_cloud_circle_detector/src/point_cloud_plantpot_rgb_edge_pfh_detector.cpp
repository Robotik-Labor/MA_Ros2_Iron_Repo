...
private:
    void listener_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
	    ...
	    pcl::toROSMsg(*unorganised_edges, output_pre_segmentation);
	    ...
	    ne.setInputCloud(unorganised_edges);
	    ...
	    pfh_estimator.setInputCloud(unorganised_edges);  // Set input cloud
	    ...
	    RCLCPP_INFO(this->get_logger(), "Target points points: %zu", unorganised_edges->points.size());
	    ...
	    sac_ia.setInputTarget(unorganised_edges);  // The filtered scene point cloud
