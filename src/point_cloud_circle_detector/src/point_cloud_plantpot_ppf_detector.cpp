...
#include <pcl/features/ppf.h>  // Include for Point Pair Features
...
void listener_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
	{
	    ...
	    pcl::PointCloud<pcl::PPFSignature>::Ptr cloud_filtered_ppf_features(new pcl::PointCloud<pcl::PPFSignature>);
	    ppf_estimator.setInputCloud(cloud_filtered_voxel);  // Set input cloud
	    ppf_estimator.setInputNormals(normals);  // Set input normals
	    ppf_estimator.compute(*cloud_filtered_ppf_features);  // Compute PPF features
	    end_time = std::chrono::high_resolution_clock::now(); // End timing
	    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
	    ...
	    voxel_object_grid.setLeafSize(0.02f, 0.02f, 0.02f);
	    voxel_object_grid.filter(*object_voxel);
	    end_time = std::chrono::high_resolution_clock::now(); // End timing
	    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
	    RCLCPP_INFO(this->get_logger(), "Voxel grid filtering took %f ms", duration.count());

	    // Step 7: Compute PPF features for the source (custom object) point cloud
	    start_time = std::chrono::high_resolution_clock::now(); // Start timing
	    ...
	    pcl::PPFEstimation<pcl::PointXYZ, pcl::Normal, pcl::PPFSignature> ppf_custom;
	    pcl::PointCloud<pcl::PPFSignature>::Ptr custom_object_ppf_features(new pcl::PointCloud<pcl::PPFSignature>);
	    ppf_custom.setInputCloud(object_voxel);
	    ppf_custom.setInputNormals(custom_object_normals);
	    //ppf_custom.setKSearch (1);
	    ppf_custom.setRadiusSearch(0.00000003);
	    ppf_custom.compute(*custom_object_ppf_features);
	    end_time = std::chrono::high_resolution_clock::now(); // End timing
	    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
	    ...
	    RCLCPP_INFO(this->get_logger(), "Source feature points: %zu", custom_object_ppf_features->points.size());
	    RCLCPP_INFO(this->get_logger(), "Target feature points: %zu", cloud_filtered_ppf_features->points.size());


	    // Step 8: RANSAC-based Model Alignment (SAC-IA) to detect the pod in the scene
	    start_time = std::chrono::high_resolution_clock::now(); // Start timing
	    pcl::SampleConsensusInitialAlignment<pcl::PointXYZ, pcl::PointXYZ, pcl::PPFSignature> sac_ia;
	    ...
	    sac_ia.setSourceFeatures(custom_object_ppf_features);  // Set the PPF features for the source
	    sac_ia.setTargetFeatures(cloud_filtered_ppf_features);  // Set the PPF features for the target
		...
		custom_object_publisher_->publish(output_aligned);
	    }
	    else
	    {
