...
class DetrObjectDetectionNode(Node):
    def __init__(self):
        ...
        self.fx_ = 907.4872436523438;  # Focal length in x direction (in pixels)
        self.fy_ = 907.2102661132812;  # Focal length in y direction (in pixels)
        self.cx_ = 651.9036254882812;  # Principal point (x)
        self.cy_ = 371.484375;         # Principal point (y)
    ...
    def image_callback(self, msg):
        try:
            ...
            for score, label, box in zip(results["scores"], results["labels"], results["boxes"]):
                ...
                y = -1*(center_x - self.cx_) * depth_value_meters / self.fx_;
                z = -1*(center_y - self.cy_) * depth_value_meters / self.fy_;
                x = depth_value_meters; # Z is simply the depth value in meters
                ...
                detection_data += f"Coordinates: ({x}, {y}, {z}) | "
