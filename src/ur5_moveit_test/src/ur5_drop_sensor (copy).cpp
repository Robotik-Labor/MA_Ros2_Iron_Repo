...
int main(int argc, char **argv) {
  ...
  visual_tools.publishText(text_pose, "Drop_Sensor_Demo", rvt::WHITE,
                           rvt::XLARGE);
  ...
  shoulder_lift_constraint.joint_name =
      "UR5_shoulder_lift_joint";                 // Name of the joint
  shoulder_lift_constraint.position = -M_PI / 2; // Desired joint position
  shoulder_lift_constraint.tolerance_above =
      M_PI / 2; // Tolerance above the desired position
  ...
  visual_tools.publishText(text_pose, "Moving_to_Drop_Down", rvt::WHITE,
                           rvt::XLARGE);
  ...
  move_group_interface.getPoseTarget(); geometry_msgs::msg::Pose target_pose2 =
  pose_stamped2.pose; move_group_interface.setPoseTarget(target_pose2);

          // Extract the quaternion components
        double qx = pose_stamped2.pose.orientation.x;
        double qy = pose_stamped2.pose.orientation.y;
        double qz = pose_stamped2.pose.orientation.z;
        double qw = pose_stamped2.pose.orientation.w;

        // Extract the position components
        double x = pose_stamped2.pose.position.x;
        double y = pose_stamped2.pose.position.y;
        double z = pose_stamped2.pose.position.z;

        // Print the values
        RCLCPP_INFO(logger, "Home position: x=%f, y=%f, z=%f", x, y, z);
        RCLCPP_INFO(logger, "Home orientation (quaternion): x=%f, y=%f, z=%f,
  w=%f", qx, qy, qz, qw);


  //plan = move_group_interface.plan()
  success = (move_group_interface.plan(plan) ==
  moveit::core::MoveItErrorCode::SUCCESS); if (success)
    {
  RCLCPP_INFO(logger, "Visualizing pre-grasp-position trajectory line");
  //visual_tools.publishAxisLabeled(pre_grasp_pose, "pre-grasp-position");
  visual_tools.publishText(text_pose, "Moving_to_Home", rvt::WHITE,
  rvt::XLARGE); visual_tools.publishTrajectoryLine(plan.trajectory,
  ...
  //move_group_interface.move();
    ...
    {
      RCLCPP_ERROR(logger, "Planning to Home position failed!");
      return 1;
    }
  move_group_interface.move();
 ...
 */
  ...
  msg.data = "Sensor_Drop_off_Complete";
