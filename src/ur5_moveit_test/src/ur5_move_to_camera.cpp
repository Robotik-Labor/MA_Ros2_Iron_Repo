...
int main(int argc, char **argv) {
  ...
  // Create a ROS logger
  auto const logger = rclcpp::get_logger("ur5_move_to_camera");
  ...
  visual_tools.publishText(text_pose, "Moving_to_Camera", rvt::WHITE,
                           rvt::XLARGE);
  ...
  visual_tools.publishText(text_pose, "Calculating_Path_Camera", rvt::WHITE,
                           rvt::XLARGE);
  visual_tools.trigger(); 


  moveit::planning_interface::MoveGroupInterface::Plan plan_camera;
  bool success = (move_group_interface.plan(plan_camera) ==
  ...
  visual_tools.publishText(text_pose, "Moving_to_Camera", rvt::WHITE,
                           rvt::XLARGE);
  visual_tools.publishTrajectoryLine(plan_camera.trajectory, joint_model_group);
  visual_tools.trigger();
  
  move_group_interface.execute(plan_camera);
  visual_tools.deleteAllMarkers();
  visual_tools.trigger();
